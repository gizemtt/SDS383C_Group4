...
```{r}
FGM = FG_mixture(train,M=3,Iter=1000,alpha=1,mm=10)
```
...
```{r}
predicted_values=plot_density_FGM(N=nrow(train),FGM)
...
fhat = kde(x=train)
plot(fhat)
kdePred = predict(fhat, x=train)
scatter3D(train[,1],train[,2],kdePred,col = "blue", theta=0, phi = 60, cex =0.5, pch=15, main = "KDE")
```
