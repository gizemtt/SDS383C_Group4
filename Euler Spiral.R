...
# get spiral data
library(readr)
datax <- read_csv("~/R/RStudio Works/SDS R/SDS383C_Group4-main/spiraly.csv")
x_train <- data.frame(datax + rnorm(n=1195,mean=0, sd = 0.001)) #add gaussian noise with sd 0.001
plot(x_train, col='red')

#Split data
#library(caTools)
#set.seed(42069)
#sample = sample.split(x[,1], SplitRatio = .625)
#x_train = subset(x, sample == TRUE)
#x_test  = subset(x, sample == FALSE)
#plot(x_train)

## Density estimation using FG-mixture
#M <- 25 #number of von Mises-Fisher kernels within each sphere
#alpha <- 10 #precision & If the sample size n<1000, then ??=10
#mm <- 10 #number of empty spheres
...
# plot
plot(x_train, col='red',xlim=c(0, 0.8), ylim=c(0, 0.8), main="Data Set")
scatter2D(pred_with_error[,1],pred_with_error[,2],col="red",
          main="FG Mixture",
          cex.symbols = .1,lwd=1,theta=0,phi =0,xlim=c(0, 0.8), ylim=c(0, 0.8))

#KDE
library(ks)
fhat = kde(x=x_train)
plot(fhat, col='red',xlim=c(0, 0.8), ylim=c(0, 0.8))
kdePred = predict(fhat, x=x_train)
scatter2D(x_train[,1], kdePred,col = "red", theta=0, phi = 0, cex =0.5, pch=15, main = "KDE")